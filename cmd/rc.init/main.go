...
func doMounts() {
	...
	mount("proc", "/proc", "proc", nodev|nosuid|noexec|relatime|unix.MS_NOATIME, "")
	...
	// mount devfs
	mount("dev", "/dev", "devtmpfs", nosuid|noexec|relatime, "size=7607628k,nr_inodes=1901907,mode=755")
	// already done in wsl2
	...
	//mount("devpts", "/dev/pts", "devpts", noexec|nosuid|unix.MS_NOATIME, "gid=5,mode=0620,ptmxmode=000")

	// sysfs
	mount("sysfs", "/sys", "sysfs", noexec|nosuid|nodev|unix.MS_NOATIME, "")
	// some of the subsystems may not exist, so ignore errors
	mountSilent("securityfs", "/sys/kernel/security", "securityfs", noexec|nosuid|nodev|unix.MS_NOATIME, "")
	mountSilent("debugfs", "/sys/kernel/debug", "debugfs", noexec|nosuid|nodev|unix.MS_NOATIME, "")
	mountSilent("configfs", "/sys/kernel/config", "configfs", noexec|nosuid|nodev|unix.MS_NOATIME, "")
	mountSilent("fusectl", "/sys/fs/fuse/connections", "fusectl", noexec|nosuid|nodev|unix.MS_NOATIME, "")
	mountSilent("selinuxfs", "/sys/fs/selinux", "selinuxfs", noexec|nosuid|unix.MS_NOATIME, "")
	mountSilent("pstore", "/sys/fs/pstore", "pstore", noexec|nosuid|nodev|unix.MS_NOATIME, "")
	mountSilent("efivarfs", "/sys/firmware/efi/efivars", "efivarfs", noexec|nosuid|nodev|unix.MS_NOATIME, "")
	...
	mount("cgroup", "/sys/fs/cgroup", "tmpfs", nodev|noexec|nosuid|relatime, "mode=755")
	// mount cgroups filesystems for all enabled cgroups
	for _, cg := range cgroupList() {
		path := filepath.Join("/sys/fs/cgroup", cg)
		mkdir(path, 0555)
		mount(cg, path, "cgroup", noexec|nosuid|nodev|relatime, cg)
	}

	// use hierarchy for memory (does not work on wsl2)
...
func main() {
	...
	// fix for wsl, there is a /dev/null with 0660 perm in the generated iso
	// somehow it has side effect everywhere
